...
.directive('uiBreadcrumb', ['$rootScope', '$state', function ($rootScope, $state) {
    return {
        ...
        template: '<ul class="breadcrumb">' +
            '<li><i class="ace-icon {{home}} home-icon"></i> <a href="">主页</a></li> ' +
        ...
        link: function (scope) {
            ...
            $rootScope.$on('$stateChangeSuccess', function (event, toState, toParams, fromState, fromParams) {
                ...
                while ((currentName = getParentName(currentName))) {
                    ...
                    activeItems[currentName] = true;
                    breadcrumbs.list.push({ title: state.title, url: state.abstract ? false : state.name });
...
.directive('tmpl', function ($compile, $parse) {
    return {
        restrict: 'A',
        transclude: true,
        templateUrl: function (element, attrs) {
            return attrs.templaturl;
        },
        link: function (scope, element, attrs) {
...
.directive('multiselectDropdown', [function () {
    return function (scope, element, attributes) {
        ...
        element.multiselect({
            ...
            selectAllText: '全选',
            nonSelectedText: '请选择需要添加的项目',
            nSelectedText: '个已经被选中',
            allSelectedText: '已全选',
            numberDisplayed: 8,
